...
class RSAAvatar(pb.Avatar):
    """
    Avatar for authenticating with RSA key pairs.  The Avatar lives on the
    server.  It is the equivilant of a session object.  It maintains
    authentication and other state for the connected client.
    ...
    def perspective_exchange_keys(self, client_pub_key):
        """
        Exchange public keys with the client.  This returns the key encrypted
        using the clients public key
        ...
        #send the servers public key.  serialize it and encrypt it
        #the key must be broken into chunks for it to be signed
        #for ease recompiling it we'll store the chunks as a list
        return [self.client_key.encrypt(i, None) for i in self.chunks()]

    def chunks(self):
        """
        Returns the public key as a list of 128 byte chunks.
        """
        json_key = simplejson.dumps(self.server_pub_key)
        chunk = self.key_size/32
        for i in range(int(math.ceil(len(json_key)/(chunk*1.0)))):
            yield json_key[i*chunk:i*chunk+chunk]

    def perspective_get_key(self):
        """
        Returns the public key of the avatar.  This returns the key unencrypted
        """
...
class RSAClient(object):
    """
    Class encompassing the client's part of the RSA handshake.  This code lives
    on the Client.
